...
export default function UNOGame({ roomCode, username, players, initialGameState, onLeaveRoom }) {
  ...
  const handlePlayCard = (cardIndex) => {
    if (iHaveFinished) {
      ...
      showToast('You have already finished this game! Wait for others to complete.', 'info');
      return;
    }
    ...
    if (currentPlayer !== username) {
      ...
      showToast(`It's not your turn! Waiting for ${currentPlayer}...`, 'warning');
      return;
    }
    ...
    <div className={`min-h-screen p-4 md:p-8 relative ${colors.bg} transition-colors duration-300`}>
      ...
      <div className="absolute top-4 right-20 flex gap-2 z-10">
        <button
          ...
          className={`px-3 py-2 rounded-xl text-sm font-bold transition-all backdrop-blur-xl ${useTableView ? 'bg-green-600 text-white' : 'bg-gray-700 text-white'
            }`}
          title="Toggle Table View"
        >
          {useTableView ? '🎴 Table' : '📋 List'}
        </button>

        <button
          onClick={() => setShowWinProbability(!showWinProbability)}
          className={`px-3 py-2 rounded-xl text-sm font-bold transition-all backdrop-blur-xl ${showWinProbability ? 'bg-blue-600 text-white' : 'bg-gray-700 text-white'
            }`}
          title="Toggle Win Probability"
        >
          📊
        </button>
      </div>

      {/* Conditional: Table View or Original View */}
      {useTableView && gameState && myHand.length > 0 ? (
        <UNOTable
          gameState={gameState}
          myHand={myHand}
          username={username}
          ...
          showProbability={showWinProbability}
          showToast={showToast}
        />
      ) : (
